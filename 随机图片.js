...
export class RandomImage extends plugin {
  ...
  async addImage() {
    ...
    try {
      ...
      let sourceType = ""; // 记录图片来源类型

      // 模式1：引用消息中的图片（最高优先级）
      if (this.e.source) {
        // 获取被引用的历史消息
        const history = this.e.isGroup
          ? await this.e.group.getChatHistory(this.e.source.seq, 1)
          : await this.e.friend.getChatHistory(this.e.source.time, 1);

        if (!history || history.length === 0) {
          return this.reply("❌ 无法获取被引用的消息");
        }

        const sourceMsg = history.pop();
        const sourceImages = sourceMsg.message.filter(
          (i) => i.type === "image"
        );

        if (sourceImages.length === 0) {
          return this.reply("❌ 被引用的消息中没有图片");
        }
  ...
  getUniqueFilePath(originalPath) {
    ...
    let newPath = path.join(
      dir,
      `${base}_${Date.now().toString(36).slice(-4)}${ext}`
    );
  ...
  async renameImage() {
    try {
      ...
      if (!match)
        return this.reply("❌ 格式错误，正确格式：&重命名图片 编号 新名称");
    ...
    try {
      ...
      const validFiles = files.filter((file) => {
        try {
          ...
          return (
            this.allowedTypes.includes(ext) &&
  ...
  errorTranslator(err) {
    const errors = {
      ...
      HTTP_500: "🛑 服务器错误(500)",
      TIMEOUT: "⏳ 请求超时",
      FILE_SIZE_EXCEEDED: `❌ 文件大小超过限制（最大 ${this.formatSize(
        this.maxFileSize
      )}）`,
